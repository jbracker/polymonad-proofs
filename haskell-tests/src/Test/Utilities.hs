...
checkAll2 :: forall a b ct.
  ( Function a, CoArbitrary a, Arbitrary a, Typeable a, Show a, ct a
  , Function b, CoArbitrary b, Arbitrary b, Typeable b, Show b, ct b
  ) => Proxy ct -> Proxy (a, b)
    -> (forall c d. ( Function c, CoArbitrary c, Arbitrary c, Typeable c, Show c, ct c
                    , Function d, CoArbitrary d, Arbitrary d, Typeable d, Show d, ct d
                    ) => Proxy (c, d) -> IO Result)
    -> IO Result
checkAll2 _ _ chck 
     = chck (Proxy :: Proxy (a,b))
  &&&& chck (Proxy :: Proxy (b,a))
  &&&& chck (Proxy :: Proxy (a,a))
  &&&& chck (Proxy :: Proxy (b,b))

checkAll3 :: forall ct a b c. 
  ( Function a, CoArbitrary a, Arbitrary a, Typeable a, Show a, ct a
  , Function b, CoArbitrary b, Arbitrary b, Typeable b, Show b, ct b
  , Function c, CoArbitrary c, Arbitrary c, Typeable c, Show c, ct c
  ) => Proxy ct -> Proxy (a,b,c) 
    -> (forall d e f. ( Function d, CoArbitrary d, Arbitrary d, Typeable d, Show d, ct d
                      , Function e, CoArbitrary e, Arbitrary e, Typeable e, Show e, ct e
                      , Function f, CoArbitrary f, Arbitrary f, Typeable f, Show f, ct f
                      ) => Proxy (d,e,f) -> IO Result) 
  -> IO Result
checkAll3 _ _ chck = chck (Proxy :: Proxy (a,b,c))
