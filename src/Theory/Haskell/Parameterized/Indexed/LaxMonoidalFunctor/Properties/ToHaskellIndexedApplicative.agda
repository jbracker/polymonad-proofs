...
IndexedLaxMonoidalFunctor→HaskellIndexedApplicative {ℓIxs} {Ixs} FMon = F' , indexed-applicative pure _<*>_ HaskFunc law-id law-composition law-homomorphism law-interchange law-applicative-fmap
  where
    ...
    fmap {i} {j} {α} {β} f Fα = Functor.F₁ (F i j) f Fα
    ...
    _**_ {i} {j} {k} {α} {β} Fα Fβ = μ i j k α β (Fα , Fβ)
    ...
    HaskFunc i j = Functor→HaskellFunctor (F i j)
    
    abstract
      nat : {i j k : Ixs} {a b : Category.Obj (SetCat' ×C SetCat')}
          → {f : Category.Hom (SetCat' ×C SetCat') a b}
          → (fmap (proj₁ f *** proj₂ f)) ∘F (nat-η (monNatTrans i j k) a)
          ≡ (nat-η (monNatTrans i j k) b) ∘F (fmap (proj₁ f) *** fmap (proj₂ f))
      nat {i} {j} {k} {a} {b} {f} = natural (monNatTrans i j k) {a} {b} {f}
    ...
    abstract
      ...
      law-naturality {i} {j} {k} {α} {β} {γ} {δ} f g u v = begin
        ( fmap (f *** g) ∘F (nat-η (monNatTrans i j k) (α , γ)) ) (u , v)
          ≡⟨ cong (λ X → X (u , v)) (nat {f = (f , g)}) ⟩
        nat-η (monNatTrans i j k) (β , δ) (fmap f u , fmap g v) ∎
    
    abstract
      law-left-identity' : {i j : Ixs} {α : Type} → (v : F' i j α) 
                         → fmap proj₂ (unit' ** v) ≡ v
      law-left-identity' {i} {j} {α} v = begin
        (fmap proj₂ ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x)) ) (lift tt , v) 
    ...
    abstract
      ...
      law-left-identity {i} {j} {α} v = begin
        (nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)
          ≡⟨ cong (λ X → (X ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)) (sym (Functor.id (F i j))) ⟩
        (fmap (λ x → x) ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)
          ≡⟨ refl ⟩
        (fmap ((λ a → (lift tt , a)) ∘F proj₂) ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)
          ≡⟨ cong (λ X → (X ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)) (Functor.compose (F i j)) ⟩
        (fmap (λ a → (lift tt , a)) ∘F fmap proj₂ ∘F nat-η (monNatTrans i i j) (Lift ⊤ , α) ∘F (ε i *** (λ x → x))) (lift tt , v)
    ...
    abstract
      law-right-identity' : {i j : Ixs} {α : Type} → (v : F' i j α) 
                          → fmap proj₁ (v ** unit') ≡ v
      law-right-identity' {i} {j} {α} v = cong (λ X → X (v , lift tt)) (sym (right-unitality α))
    
    abstract
      law-right-identity : {i j : Ixs} {α : Type} → (v : F' i j α) 
                         → (v ** unit') ≡ fmap (λ a → (a , lift tt)) v
      law-right-identity {i} {j} {α} v = begin
        (nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)
          ≡⟨ cong (λ X → (X ∘F nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)) (sym $ Functor.id (F i j)) ⟩
        (fmap (λ x → x) ∘F nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)
          ≡⟨ refl ⟩
        (fmap ((λ a → (a , lift tt)) ∘F proj₁) ∘F nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)
          ≡⟨ cong (λ X → (X ∘F nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)) (Functor.compose (F i j)) ⟩
        (fmap (λ a → (a , lift tt)) ∘F fmap proj₁ ∘F nat-η (monNatTrans i j j) (α , Lift ⊤) ∘F ((λ x → x) *** ε j)) (v , lift tt)
    ...
    abstract
      ...
      law-applicative-fmap {i} {j} {α} {β} f x = begin
        ...
        fmap ((λ x → f (proj₂ {a = zero} x)) ∘F (λ a → (lift tt , a))) x
          ≡⟨ cong (λ X → X x) (Functor.compose (F i j)) ⟩
        ...
        fmap ( (λ {(f , a) → f a}) ∘F ((λ _ → f) *** (λ x → x)) ) (ε i (lift tt) ** x)
          ≡⟨ cong (λ X → X (ε i (lift tt) ** x)) (Functor.compose (F i j)) ⟩
        ...
        fmap (λ {(f , a) → f a}) ((fmap (λ _ → f) (ε i (lift tt))) ** fmap (λ x → x) x)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) ((fmap (λ _ → f) (ε i (lift tt))) ** X)) (cong (λ X → X x) (Functor.id (F i j))) ⟩
    ...
    abstract
      ...
      law-associativity {α} {β} {γ} {i} {j} {k} {l} u v w = sym $ begin
        (nat-η (monNatTrans i j l) (α , (β × γ)) ∘F (idF *** nat-η (monNatTrans j k l) (β , γ))) (u , (v , w)) 
          ≡⟨ cong (λ X → X ((u , v) , w)) (sym $ assoc FMon α β γ) ⟩
        (fmap (λ {((a , b) , c) → (a , (b , c))}) ∘F nat-η (monNatTrans i k l) ((α × β) , γ) ∘F (nat-η (monNatTrans i j k) (α , β) *** idF)) ((u , v) , w) ∎
    ...
    abstract
      ...
      law-composition {α} {β} {γ} {i} {j} {k} {l} u v w = begin
        fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap (λ {(f , a) → f a}) (fmap (λ _ → _∘′_) unit' ** u)) ** v) ** w) 
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap (λ {(f , a) → f a}) (fmap (λ _ → _∘′_) unit' ** X u)) ** v) ** w)) (sym $ Functor.id (F i j)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap (λ {(f , a) → f a}) (fmap ((λ _ → _∘′_) *** idF) (fmap (λ a → (lift tt , a)) u))) ** v) ** w) 
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((X (fmap (λ a → (lift tt , a)) u)) ** v) ** w)) (sym $ Functor.compose (F i j)) ⟩
        fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap ((λ {(f , a) → f a}) ∘F ((λ _ → _∘′_) *** idF)) (fmap (λ a → (lift tt , a)) u)) ** v) ** w)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((X u) ** v) ** w)) (sym $ Functor.compose (F i j)) ⟩
        fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap g u) ** v) ** w)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) ((fmap g u) ** X v) ** w)) (sym $ Functor.id (F j k)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap (λ {(f , a) → f a}) (fmap (g *** idF) (u ** v)) ** w)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (X (u ** v) ** w)) (sym $ Functor.compose (F i k)) ⟩
        fmap (λ {(f , a) → f a}) (fmap ((λ {(f , a) → f a}) ∘F (g *** idF)) (u ** v) ** w)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap ((λ {(f , a) → f a}) ∘F (g *** idF)) (u ** v) ** X w)) (sym $ Functor.id (F k l)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap (((λ {(f , a) → f a}) ∘F (g *** idF)) *** idF) ((u ** v) ** w))
          ≡⟨ cong (λ X → X ((u ** v) ** w)) (sym $ Functor.compose (F i l)) ⟩
        fmap h ((u ** v) ** w)
          ≡⟨ refl ⟩
        fmap (h' ∘F (λ {((a , b) , c) → (a , (b , c))})) ( (u ** v) ** w )
          ≡⟨ cong (λ X → X ( (u ** v) ** w )) (Functor.compose (F i l)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap idF u ** fmap (λ {(f , a) → f a}) (v ** w))
          ≡⟨ cong (λ X → fmap (λ { (f , a) → f a }) (X u ** fmap (λ { (f , a) → f a }) (v ** w))) (Functor.id (F i j)) ⟩
    ...
    abstract
      ...
      law-homomorphism {i} {α} {β} x f = begin
        ...
        fmap (λ {(f , a) → f a}) (fmap ((λ _ → f) *** (λ _ → x)) (unit' ** unit'))
          ≡⟨ cong (λ X → X (unit' ** unit')) (sym $ Functor.compose (F i i)) ⟩
        ...
        fmap ((λ {(f , a) → f a}) ∘F ((λ _ → f) *** (λ _ → x))) (fmap (λ a → (lift tt , a)) unit')
          ≡⟨ cong (λ X → X unit') (sym $ Functor.compose (F i i)) ⟩
        fmap (λ _ → f x) unit' ∎
    ...
    abstract
      ...
      law-interchange {i} {j} {α} {β} u x = begin
        fmap (λ {(f , a) → f a}) (u ** fmap (λ _ → x) unit')
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (X u ** fmap (λ _ → x) unit')) (sym $ Functor.id (F i j)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap (idF *** (λ _ → x)) (u ** unit'))
          ≡⟨ cong (λ X → X (u ** unit')) (sym $ Functor.compose (F i j)) ⟩
        ...
        fmap ((λ {(f , a) → f a}) ∘F (idF *** (λ _ → x))) (fmap (λ a → (a , lift tt)) u)
          ≡⟨ cong (λ X → X u) (sym $ Functor.compose (F i j)) ⟩
        fmap ((λ {(f , a) → f a}) ∘F ((λ _ → (λ f → f x)) *** idF) ∘F (λ a → (lift tt , a))) u
          ≡⟨ cong (λ X → X u) (Functor.compose (F i j)) ⟩
        ...
        fmap ((λ {(f , a) → f a}) ∘F ((λ _ → (λ f → f x)) *** idF)) (unit' ** u)
          ≡⟨ cong (λ X → X (unit' ** u)) (Functor.compose (F i j)) ⟩
        ...
        fmap (λ {(f , a) → f a}) (fmap (λ _ → (λ f → f x)) unit' ** fmap idF u)
          ≡⟨ cong (λ X → fmap (λ {(f , a) → f a}) (fmap (λ _ → (λ f → f x)) unit' ** X u)) (Functor.id (F i j)) ⟩
