...
module Theory.Haskell.Constrained.Examples.SetFunctor.Base where 
...
record LSet (A : Σ Type (OrdInstance {lzero} {lzero})) : Set lzero where
  constructor lset
  field
    xs : List (proj₁ A) 
    sorted : IsSortedNoDupList (proj₂ A) xs

lset-eq : {A : Type} {OrdA : OrdInstance {lzero} {lzero} A} 
        → (xs ys : List A)
        → (sortedX : IsSortedNoDupList OrdA xs) → (sortedY : IsSortedNoDupList OrdA ys)
        → xs ≡ ys
        → lset xs sortedX ≡ lset ys sortedY
lset-eq {A} {OrdA} xs .xs sortedX sortedY refl with proof-irr-IsSortedNoDupList OrdA xs sortedX sortedY
lset-eq {A} {OrdA} xs .xs sortedX .sortedX refl | refl = refl 
