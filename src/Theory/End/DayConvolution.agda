...
dayConvolution = functor day₀ day₁ {!!} {!!}
  where
    ...
    cowed : (F G : Functor C Set') → {a b : Obj C} → Hom C a b → CoWedge (dayF F G a) (Set-co-∫ ℓSet (dayF F G b))
    cowed F G {a} {b} f = cowedge {!CoWedge.co-e $ CoWedge-∫ ℓSet (dayF F G a)!} {!!}
    ...
    day₀ (F , G) = functor d0₀ d0₁ {!!} {!!}
      where
        ...
        d0₁ {a} {b} f = {!!} -- proj₁ $ universal-co-∫ ℓSet (dayF F G a) {!!}
