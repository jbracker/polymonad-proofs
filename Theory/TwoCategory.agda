...
record LaxTwoCategory {ℓ₀ ℓ₁ ℓ₂ : Level} : Set (lsuc (ℓ₀ ⊔ ℓ₁ ⊔ ℓ₂)) where
  ...
  lUnitor : {a b : Cell₀} {f : Cell₁ a b} → Cell₂ (id₁ ∘ₕ f) f
  lUnitor {a} {b} {f} = subst₂ Cell₂ (sym hIdL₁) refl id₂
  
  lUnitorInv : {a b : Cell₀} {f : Cell₁ a b} → Cell₂ f (id₁ ∘ₕ f)
  lUnitorInv {a} {b} {f} = subst₂ Cell₂ refl (sym hIdL₁) id₂
  
  rUnitor : {a b : Cell₀} {f : Cell₁ a b} → Cell₂ (f ∘ₕ id₁) f
  rUnitor {a} {b} {f} = subst₂ Cell₂ (sym hIdR₁) refl id₂ 

  rUnitorInv : {a b : Cell₀} {f : Cell₁ a b} → Cell₂ f (f ∘ₕ id₁)
